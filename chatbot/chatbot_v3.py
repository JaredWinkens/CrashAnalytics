...
def get_table_schema_dict(db_path: str, table_name: str, table_desc: str, include_samples: bool = True, sample_limit: int = 1) -> dict:
    ...
    lines = [f"Table Name: {schema['table_name']}", f"Table Description: {schema['table_desc']}", "Columns:"]
...
class RoadSafetyChatbot():
    ...
    def get_agent_response(self, user_query: str) -> BotResponse:
        """
        Gemini decides which tool to use (SQL, RAG, or VIS) and executes it.
        """
        ...
        try:
            ...
            for tool in tools_to_use:
                ...
                if tool.function_name == 'execute_sql_query':
                    ...
                    tool_responses.append({"Tool_Name": tool.function_name, "Result": self._get_sql_tool_response(sql_query, user_query)})

                elif tool.function_name == 'retrieve_relevant_chunks':
                    rag_query = tool.function_arguments[0].strip("'\"") # Remove potential quotes
                    tool_responses.append({"Tool_Name": tool.function_name, "Result": self._get_rag_tool_response(rag_query, user_query)})
                    ...
                    tool_responses.append({"Tool_Name": tool.function_name, "Result": self._get_visualization_tool_response(location_list, filter_query, user_query)})
    ...
    def _get_sql_tool_response(self, sql_query: str, user_query: str):
        print(f"Executing SQL Query: {sql_query}")
        tool_result = self._execute_sql_query(sql_query)
    ...
    def _get_rag_tool_response(self, rag_query: str, user_query: str):    
        print(f"Executing RAG Query for: {rag_query}")
        tool_result = self._retrieve_relevant_chunks(rag_query)
    ...
    def _get_visualization_tool_response(self, location_list: list, filter_query: str, user_query: str):
        ...
        tool_result = self._visualize_data(location_list, filter_query)
    ...
    def _retrieve_relevant_chunks(self, query: str, top_k: int = 30):
        """
